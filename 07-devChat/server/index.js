...
// ==========================================
// Este servidor gerencia as conexóes de usuários e distribui mensagens
// Tecnologias:
// - Express: Framework web para HTTP
// - Socket.io: Comunicação bidirecional em tempo real via WebSocket

const app = require("express")(); // Importa a bibilioteca Express
const server = require("http").createServer(app); // Importa módulo HTTP nativo do Node.js (necessário para Socket.io)
...
io.on("connection", (socket) => {
  ...
  // socket.id: ID único do cliente cliente (gerado automaticamente)
  ...
  socket.on("set_username", (username) => {
    // Armazena op nome de usuário no objeto socket para uso posterior
  ...
  socket.on("message", (text) => {
    ...
    // 2. Envia para TODOS os conectados usando io.emit()
